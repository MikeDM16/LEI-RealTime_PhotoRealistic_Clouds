...
void getOpticalDepth(float height, float angle, float height2, out float hr, out float hm) {

	float stepH = atmosRadius - earthRadius;
	float stepAngle = PI *0.60;

	float aux = height;
	vec2 totalOD = texture(texUnit, vec2(aux / stepH, angle/stepAngle)).xy;

	aux = height2;// - earthRadius;
	vec2 partOD = texture(texUnit, vec2(aux / stepH, angle/stepAngle)).xy;
	
	vec2 finalOD = totalOD - partOD;
...
void getSingleOpticalDepth(float height, float angle, out float hr, out float hm) {
	...
	float stepH = atmosRadius - earthRadius;
	float stepAngle = PI *0.80;
	
	vec2 totalOD;
	
	if (angle <= PI*0.80)
		totalOD = texture(texUnit, vec2(height / stepH , angle/stepAngle)).xy;
	else
		totalOD = vec2(-1);
...
/*vec3 skyColor(vec3 dir, vec3 sunDir, vec3 origin, float dist) {
	...
	float angle = (90 - sunAngles.y)*PI/180;
	...
	for(int i = 0; i < divisions; ++i) {
		...
		if (height < 0) {
			return vec3(0.0,0.70,0.0);
			break;
		}
		
		hr = exp(-height / Hr) * segLength;
		hm = exp(-height / Hm) * segLength;
		...
		//getSingleOpticalDepth(height, angle, opticalDepthLightR,opticalDepthLightM);
		float dista = distToTopAtmosphere(samplePos, sunDir);
		vec2 od =computeOpticalDepth(samplePos, dista,  sunDir);
		opticalDepthLightR = od[0];
		opticalDepthLightM = od[1];
		
		if (opticalDepthLightM != -1 ) {
