...
vec3 skyColor(vec3 dir, vec3 sunDir, vec3 origin, float dist) {
	...
	float quotient =  pow(dist, 1.0/(divisions));
	float segLength;// = quotient;
	float current = 1;
	float height;
	for(int i = 0; i < divisions; ++i) {
		...
		float quotientLight =  pow(distLightRay, 1.0/(divisonsLightRay));
		float segLengthLight = 1;
		float currentLight = 1;
		float opticalDepthLightR = 0;
		float opticalDepthLightM = 0;
		int j = 0;
		for (; j < divisonsLightRay; ++j) {
			segLengthLight = currentLight * quotientLight - currentLight;
